...
const GroupManager = {
    ...
    init: async function () {
        ...
        this.elements.tabGroupTemplate =
            document.getElementById("tab-group-template");
        this.elements.tabItemTemplate =
        ...
        this.elements.tabGroups.addEventListener(
            "click",
            this.handleTabGroupsClick.bind(this)
        );
    ...
    renderGroups: function () {
        ...
        if (Object.keys(this.state.groups).length === 0) {
            ...
            noGroupsMessage.textContent =
                "No tab groups found. Click refresh to categorize tabs.";
        ...
        // Add all groups to the DOM at once
        this.elements.tabGroups.appendChild(fragment);

        // Add scroll indicator back (it was removed when clearing innerHTML)
        ...
        `;
        this.elements.tabGroups.appendChild(scrollIndicator);

        // Check if we need to show the scroll indicator
        setTimeout(() => {
            const { scrollHeight, clientHeight } = this.elements.tabGroups;
    ...
    createGroupElement: function (groupName, tabs) {
        ...
        // Add each tab to the fragment
        tabs.forEach((tab, index) => {
            const tabElement = this.createTabElement(tab);

            // Add a special class to the last tab item
            if (index === tabs.length - 1) {
                tabElement.classList.add("last-tab-item");
            }

            tabsFragment.appendChild(tabElement);
        });

        // Add all tabs to the container at once
    ...
    createTabElement: function (tab) {
        ...
        // Add URL as data attribute for debugging
    ...
    removeTabFromGroups: async function (tabId) {
        ...
        for (const groupName in this.state.groups) {
            const index = this.state.groups[groupName].findIndex(
                (tab) => tab.id === tabId
            );
            if (index !== -1) {
